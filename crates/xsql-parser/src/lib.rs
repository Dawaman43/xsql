...
pub mod v2 {
	...
	pub fn parse_postgres_schema_v2(sql: &str) -> Result<V2Schema, String> {
		...
		for stmt in ast {
			if let Statement::CreateTable { name, columns, constraints, .. } = stmt {
				...
				for col in columns {
					...
					for opt in &col.options {
						match &opt.option {
							ColumnOption::NotNull => col_constraints.push(ColumnConstraint::NotNull),
							ColumnOption::Null => {},
							ColumnOption::Default(expr) => col_constraints.push(ColumnConstraint::Default(expr.to_string())),
							ColumnOption::Unique { is_primary: true, .. } => {
								// PK handled below
							}
							ColumnOption::Generated { .. } => col_constraints.push(ColumnConstraint::AutoIncrement),
							ColumnOption::Check(expr) => col_constraints.push(ColumnConstraint::Check(expr.to_string())),
					...
					} else if dt.contains("double") || dt.contains("float8") || dt.contains("double precision") {
						V2DataType::Double
					} else if dt.contains("float") || dt.contains("real") {
						V2DataType::Float
					} else if dt.contains("varchar") || dt.contains("character varying") || dt.contains("char") {
						// try to extract size
						let size = dt.split(|c: char| !c.is_digit(10)).filter_map(|s| s.parse::<u32>().ok()).next();
				...
				for c in constraints {
					match c {
						TableConstraint::Unique { name, is_primary, columns, .. } => {
							if is_primary {
	...
	pub fn parse_mysql_schema_v2(sql: &str) -> Result<V2Schema, String> {
		...
		fn has_auto_increment(tokens: &[Token]) -> bool {
			tokens.iter().any(|t| matches!(t, Token::Word(w) if w.value.eq_ignore_ascii_case("auto_increment")))
		}
		for stmt in ast {
			if let Statement::CreateTable { name, columns, constraints, .. } = stmt {
				let mut table = V2Table {
